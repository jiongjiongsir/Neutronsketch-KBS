...
print(data.files)
# data['edge_type'] = data['edge_type'].fill(0)
# data['edge_timestamp'] = data['edge_timestamp'].fill(0)
# print(type(data['edge_type']))
...
star = np.concatenate((train_mask, valid_mask, test_mask), axis=0)
print(star.shape)
print('before process time: ',time.time()-time1)
# In[]:
...
if args.cut == 'alpha':
    ...
    start_time = time.perf_counter()
    n_id = g.extract_skeleton(args.d[1], args.d[0], 2, True, 64)
    print(f"Done! [{time.perf_counter() - start_time:.2f}s]")
...
# In[ ]:
    ...
    tmp[star] = True
    star32 = tmp
    print(edge_index32.shape)
    g = graph_skeleton.Graph(edge_index32, star32)

    start_time = time.perf_counter()
    n_id = g.extract_skeleton(args.d[1], args.d[0], 15, True, 32)

    start_time = time.perf_counter()
    print('before reconstruct_reweighted_edge:',time.time()-time_start)
    # n_edge_index, n_edge_weight = g.reconstruct_reweighted_edge(n_id)
    n_edge_index = g.reconstruct_edge(n_id)
    n_x, cnt_x = reconstruct_x(x, n_id)
    n_y = mapping_label(y, n_id)
    ...
    g2.drop_corr()
    n_id2 = g2.extract_skeleton(1, 2, 2, True, 32)
    ...
    n_edge_index2 = g2.reconstruct_edge(n_id2)
    print(f"Done! [{time.perf_counter() - start_time:.2f}s]")
    ...
    print("Finish process time: ",time.time()-time_start)
