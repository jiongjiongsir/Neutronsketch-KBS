...
def test(model, data, masks, no_conv=False):
    ...
    accuracy_val = metrics.accuracy_score(data.y[masks[0]].cpu(), output[masks[0]])
    accuracy_test = metrics.accuracy_score(data.y[masks[1]].cpu(), output[masks[1]])
...
parser.add_argument('--cut', default='skeleton_gamma', choices = ['no', 'tg', 'bg', 'skeleton_alpha','skeleton_beta','skeleton_gamma'],
...
if args.cut != 'skeleton_gamma':
    ...
    zip = np.load('/home/liuyj/code/GraphSkeleton/datasets/ogbn-arxiv/skeleton/skeleton_gamma.npy', allow_pickle=True).item()
