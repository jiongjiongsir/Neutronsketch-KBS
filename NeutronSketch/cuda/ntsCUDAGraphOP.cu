...
void Cuda_Stream::zero_copy_feature_move_gpu_test(float* dev_feature, float* host_pinned_feature, std::vector<int> nid_vec,
                                             VertexId_CUDA feature_size, VertexId_CUDA vertex_size) {
VertexId_CUDA *src_vertex = NULL;
cudaMalloc((void**)&src_vertex,nid_vec.size()*sizeof(VertexId_CUDA));
printf("test cuda1?\n");
move_bytes_in(src_vertex, nid_vec.data(), nid_vec.size() * sizeof(int));
printf("test cuda2?\n");                                             
#if CUDA_ENABLE
