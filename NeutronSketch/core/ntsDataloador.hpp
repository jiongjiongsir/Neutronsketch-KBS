 ...
 public:
  ...
  void readFeature_Label_Mask(std::string inputF, std::string inputL, std::string inputM) {
    ...
    for(int i = 0; i<graph->vertices; i++)
    {
      if(global_mask[i]==0)
      {
        if(graph->in_degree[i]>graph->config->target_degree)
        {
          sums += 1;
          // global_mask[i] = 3;
          // local_mask[i] = 3;
    ...
    std::cout<<"degree test" << graph->vertices <<" "<<sums<<std::endl;
  ...
  void readFeature_Label_Mask_Degree(std::string inputF, std::string inputL, std::string inputM, int target_degree) {
    // logic here is exactly the same as read feature and label from file
    std::string str;
    std::ifstream input_ftr(inputF.c_str(), std::ios::in);
    std::ifstream input_lbl(inputL.c_str(), std::ios::in);
    ...
    for(int i = 0; i<graph->vertices; i++)
    {
      if(global_mask[i]==0)
      {
        if(graph->in_degree[i]>target_degree)
        {
          sums += 1;
          // global_mask[i] = 3;
          // local_mask[i] = 3;
