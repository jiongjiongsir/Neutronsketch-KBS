...
//     ////////////////////////////////////////////////////////
//     update_batch_size(whole_graph->graph_->config->batch_size, "Sampler init ");
...
//   void update_batch_size(VertexId batch_size_, std::string info="") {
//     // batch_size = whole_graph->graph_->config->batch_size;
//     batch_size = batch_size_;
//     if (work_range[1] < batch_size || full_batch) batch_size = work_range[1];
//     batch_nums = (work_range[1] + batch_size - 1) / batch_size;
//     LOG_DEBUG("%sbatch_size switch to %d, now batch_size %d, batch_num %d", info.c_str(), batch_size_, batch_size, batch_nums);
//   }
...
class Sampler {
 public:
  ...
  std::vector<VertexId> metis_partition_id;
  std::vector<std::vector<VertexId>> batch_nodes;
  std::vector<std::vector<VertexId>> partition_nodes;
  VertexId batch_size;
  VertexId batch_nums;
  VertexId layers;
  VertexId metis_batch_id;
  ...
  Sampler(FullyRepGraph* whole_graph_, std::vector<VertexId>& index, bool full_batch = false) {
    ...
    // this->device = dev;
    // this->gpu_id = gpu_id;
    // printf("vertices %d", whole_graph->global_vertices);
    cs = new Cuda_Stream();
    // assert(index.size() > 0);
    // printf("vertices %d", whole_graph->global_vertices);
    sample_nids.assign(index.begin(), index.end());
    ...
    fanout_vec = whole_graph_->graph_->config->fanout_vec;
    fanout_switch_idx = -1;
  ...
  //     }
  //     if (!tmp.empty()) {
  //       partition_nodes.push_back(std::move(tmp));
  //     }
  //   }
  //   assert(debug_cnt == sample_nids.size());
  ...
  //   // int tmp_batch_num = 0;
  //   // int index = 0;
  //   // int cross_partition = 0;
  //   // std::vector<VertexId>tmp_vector;
  //   // for(int i=0; i<partition_nums;i++){

  //   //   if(cross_partition != 0){
  //   //     int tmp_num = batch_size-index;
  //   //     index = 0;
  //   //     for(int j=0; j<tmp_num; j++){
  //   //       tmp_vector.push_back(partition_nodes[i][j]);
  //   //       index++;
  //   //     }
  //   //     batch_nodes.push_back(tmp_vector);
  //   //     cross_partition = 0;
  //   //   }
  //   //   printf("index: %d\n",index);
  //   //   tmp_batch_num = (partition_nodes[i].size()-index)/batch_size;
  //   //   printf("partition %d nodes %d batch nums: %d\n",i,partition_nodes[i].size(),tmp_batch_num);
  //   //   for(int j=0;j<tmp_batch_num;j++){
  //   //     tmp_vector.clear();
  //   //     tmp_vector.assign(partition_nodes[i].begin()+index,partition_nodes[i].begin()+index+batch_size);
  //   //     index += batch_size;
  //   //     batch_nodes.push_back(tmp_vector);
  //   //   }
  //   //   if(index < partition_nodes[i].size()){
  //   //     tmp_vector.clear();
  //   //     tmp_vector.assign(partition_nodes[i].begin()+index,partition_nodes[i].end());
  //   //     index = partition_nodes[i].size() - index;
  //   //     cross_partition = 1;
  //   //   }
  ...
  void get_metis_batch() {
    batch_nodes.clear();
    shuffle_vec_seed(partition_nodes);
    for(int i=0;i<partition_nodes.size();i++) {
      shuffle_vec_seed(partition_nodes[i]);
    }
    int nodes_num = 0;
    int tmp_batch_num = 0;
    int index = 0;
    ...
    for(int i=0; i<partition_nums;i++){
      ...
      if(cross_partition != 0){
        int tmp_num = batch_size-index;
        if(tmp_num > partition_nodes[i].size()) {
          tmp_num = partition_nodes[i].size();
          // index = 0;
          for(int j=0; j<tmp_num; j++){
            tmp_vector.push_back(partition_nodes[i][j]);
            index++;
      ...
      printf("index: %d\n",index);
      if(partition_nodes[i].size() < index) {
        tmp_batch_num = 0;
      } else {
        tmp_batch_num = (partition_nodes[i].size()-index)/batch_size; 
      }
    ...
    LOG_DEBUG("metis batch num %d", batch_nodes.size());
    // shuffle()
    // shuffle_vec_seed(batch_nodes);
    for (int i = 0; i < batch_nodes.size(); ++i) {
      // shuffle_vec_seed(batch_nodes[i]);
      printf("metis batch id %d has %d nodes\n", i, batch_nodes[i].size());
    }
  }
  ...
  void update_batch_size(VertexId batch_size_) {
    // batch_size = whole_graph->graph_->config->batch_size;
    std::cout << "batch_size switch to " << batch_size_ << std::endl;
    // std::cout << work_range[1] << std::endl;
    batch_size = batch_size_;
    if (work_range[1] < batch_size || full_batch) batch_size = work_range[1];
    // std::cout << (work_range[1] + batch_size - 1) << " " <<  batch_size << std::endl;
    batch_nums = (work_range[1] + batch_size - 1) / batch_size;
  }
  ...
  bool update_fanout_from_time(float gcn_run_time) {
    int idx = gcn_run_time / whole_graph->graph_->config->fanout_switch_time;
    LOG_DEBUG("gcn_time %.3f switch_time %.3f idx %d fanout_idx %d fanout_vec size %d", gcn_run_time, whole_graph->graph_->config->fanout_switch_time, idx, fanout_switch_idx, fanout_vec.size());
    if (idx != fanout_switch_idx && idx * 2 + 1 < fanout_vec.size()) {
      fanout_switch_idx = idx;
      for (int i = 0; i < layers; ++i) {
        whole_graph->graph_->gnnctx->fanout[i] = fanout_vec[fanout_switch_idx * 2 + i];
      }
      update_fanout(whole_graph->graph_->gnnctx->fanout);
      return true;
    }
    return false;
  }
  ...
  bool update_batch_size_from_acc(int epoch, float val_acc, float gcn_run_time) {
    ...
    if (best_acc_epoch == -1) {
      ...
      if (batch_size_switch_idx + 1 < batch_size_vec.size()) {
        whole_graph->graph_->config->batch_size = batch_size_vec[++batch_size_switch_idx];
        update_batch_size(batch_size_vec[batch_size_switch_idx]);
        // for(int i=0;i<batch_size_vec.size();i++)
        // {
        //   printf("%d ",batch_size_vec[i]);
        // }
        // printf("%d ",whole_graph->graph_->config->batch_size);
        LOG_DEBUG("epoch %d best_acc_epoch %d gcn_run_time %.3f, batch_size switch to %d", epoch, best_acc_epoch,
                  gcn_run_time, whole_graph->graph_->config->batch_size);
        best_acc_epoch = epoch;
        LOG_DEBUG("epoch %d val_acc %.3f best_val_acc %.3f best_epoch %d", epoch, val_acc, best_val_acc,
  ...
  bool update_fanout_from_acc(int epoch, float val_acc, float gcn_run_time) {
    LOG_DEBUG("epoch %d val_acc %.3f best_epoch %d best_acc %.3f", epoch, val_acc, best_acc_epoch, best_val_acc);
    if (best_acc_epoch == -1) {
      best_acc_epoch = epoch;
      best_val_acc = val_acc;
    } else if (val_acc > best_val_acc) {
      best_acc_epoch = epoch;
      best_val_acc = val_acc;
    } else if (epoch - best_acc_epoch >= whole_graph->graph_->config->fanout_switch_acc) {
      if (fanout_switch_idx + 2 < fanout_vec.size()) {
        for (int i = 0; i < layers; ++i) {
          whole_graph->graph_->gnnctx->fanout[i] = fanout_vec[fanout_switch_idx + i + 1];
        }
        update_fanout(fanout_vec, fanout_switch_idx);
        fanout_switch_idx += 2;
        LOG_DEBUG("epoch %d best_acc_epoch %d gcn_run_time %.3f, fanout switch to %d", epoch, best_acc_epoch,
                  gcn_run_time, whole_graph->graph_->gnnctx->fanout);
        best_acc_epoch = epoch;
        LOG_DEBUG("epoch %d val_acc %.3f best_val_acc %.3f best_epoch %d", epoch, val_acc, best_val_acc,
        best_acc_epoch);
        return true;
  ...
  void update_fanout(std::vector<int> nums, int idx) {
    assert(idx + 1 < nums.size());
    for (int i = 0; i < layers; ++i) {
      fanout[i] = nums[idx + i + 1];
    }
    subgraph->update_fanout(fanout);
  }
  ...
  void load_feature_gpu(NtsVar& local_feature, ValueType* global_feature_buffer) {
    ...
    std::cout<<csc_layer->v_size<<csc_layer->src_size<<endl;
    ValueType* local_feature_buffer =
        whole_graph->graph_->Nts->getWritableBuffer(local_feature, torch::DeviceType::CUDA);
    cs->zero_copy_feature_move_gpu(local_feature_buffer, global_feature_buffer, csc_layer->dev_source,
                                   local_feature.size(1), csc_layer->src_size);
  ...
  void load_target_feature_gpu(Cuda_Stream* cuda_stream, NtsVar& local_feature, std::vector<int>nid_vec,
                        ValueType* global_feature_buffer) {
    // auto csc_layer = ssg->sampled_sgs[0];
    // // LOG_DEBUG("loacl_feature (%d %d) src_size %u", local_feature.size(0), local_feature.size(1),
    // // csc_layer->src_size);
    ...
    ValueType* local_feature_buffer =
        whole_graph->graph_->Nts->getWritableBuffer(local_feature, torch::DeviceType::CUDA);
        printf("test sampler?\n");
    cuda_stream->zero_copy_feature_move_gpu_test(local_feature_buffer, global_feature_buffer, nid_vec,
                                            local_feature.size(1), nid_vec.size());                         
                        }
    ...
    for (int i = 0; i < layers; i++) {
      ...
      if (i == 0) {
        if (phase && whole_graph->graph_->config->batch_type == RANDOM) {
          ...
          LOG_DEBUG("batch: work_offset %d",work_offset);
          csc_layer->init_dst(sample_nids.data() + work_offset);
        }
        work_offset += actl_batch_size;
        // LOG_DEBUG("dst size %d", csc_layer->v_size);
  ...
  void sample_one(SampledSubgraph* ssg, int type = 0, bool phase = true) {
    assert(work_offset < work_range[1]);
    // assert(batch_size == batch_size_);
    // printf("sample_bs: %d",batch_size);
    VertexId actl_batch_size = std::min(batch_size, work_range[1] - work_offset);
    if (phase && whole_graph->graph_->config->batch_type == METIS) {
      // LOG_INFO("batch nums:%d",batch_nodes.size());
      // LOG_INFO("metis_batch_id %d",metis_batch_id);
      actl_batch_size = batch_nodes[metis_batch_id].size();
      // LOG_INFO("actl_batch_size %d",actl_batch_size);
    }
    assert(actl_batch_size > 0);
    // LOG_DEBUG("actl_batch %d", actl_batch_size);
    // SampledSubgraph* ssg=new SampledSubgraph(layers,fanout_);
    ...
    for (int i = 0; i < layers; i++) {
      ...
      if (phase) { // train
          ssg->init_co(
            [&](VertexId dst) {
              ...
              if (whole_graph->graph_->config->sample_mode == 1) {
                // high + sample, low + fanout
                if (nbrs > whole_graph->graph_->config->target_degree) { // high degree
                  float sample_rate = whole_graph->graph_->config->sample_rate;
                  int lower_edge = std::min(whole_graph->graph_->config->lower_fanout, nbrs);
                  // LOG_DEBUG("sample: dst %d nbr %d target_degree %d  sample %d", dst, nbrs, whole_graph->graph_->config->target_degree, std::max(int(nbrs * sample_rate), lower_edge));
                  return std::max(int(nbrs * sample_rate), lower_edge);
                } else { // lower degree
                  int ret = -1;
                  // if (fanout[i] < 0) return nbrs;
                  // return std::min(nbrs, fanout[i]);
                  if (fanout[i] < 0) ret = nbrs;
                  else ret = std::min(nbrs, fanout[i]);
                  // LOG_DEBUG("fanout: dst %d nbr %d target_degree %d sample %d", dst, nbrs, whole_graph->graph_->config->target_degree, ret);
                  return  ret;
                }
              } else if (whole_graph->graph_->config->sample_mode == 2) {
                // high + sample, low + sample
                float sample_rate = whole_graph->graph_->config->sample_rate;
                int lower_edge = std::min(whole_graph->graph_->config->lower_fanout, nbrs);
                // LOG_DEBUG("sample: dst %d nbr %d target_degree %d  sample %d", dst, nbrs, whole_graph->graph_->config->target_degree, std::max(int(nbrs * sample_rate), lower_edge));
                return std::max(int(nbrs * sample_rate), lower_edge);
              } else if (whole_graph->graph_->config->sample_mode == 3) {
                // high + fanout, low + sample
                if (nbrs <= whole_graph->graph_->config->target_degree) { // low degree
                  float sample_rate = whole_graph->graph_->config->sample_rate;
                  int lower_edge = std::min(whole_graph->graph_->config->lower_fanout, nbrs);
                  // LOG_DEBUG("sample: dst %d nbr %d target_degree %d  sample %d", dst, nbrs, whole_graph->graph_->config->target_degree, std::max(int(nbrs * sample_rate), lower_edge));
                  return std::max(int(nbrs * sample_rate), lower_edge);
              ...
              } else if (whole_graph->graph_->config->sample_mode == 4) {
      ...
      ssg->sample_processing(
          [&](VertexId fanout_i, VertexId dst, VertexId* column_offset, VertexId* row_indices, VertexId id) {
            ...
            bool high_degree = edge_nums > whole_graph->graph_->config->target_degree;
            
            if (whole_graph->graph_->config->sample_mode == 1) {
              // 1. hidh + sample rate, low + fanout
              if (phase && high_degree) { 
                  VertexId tmp_fanout = std::max(whole_graph->graph_->config->lower_fanout,
                                              int(edge_nums * whole_graph->graph_->config->sample_rate));
                  fanout_i = tmp_fanout;
              }
            } else if (whole_graph->graph_->config->sample_mode == 2) {
              // 2. high + sample rate, low + sample rate
              if (phase) { 
                  VertexId tmp_fanout = std::max(whole_graph->graph_->config->lower_fanout,
                                              int(edge_nums * whole_graph->graph_->config->sample_rate));
                  fanout_i = tmp_fanout;
              ...
              // 4.. high + fanout, low + fanout
            } else {
              assert(false);
            }
            
            VertexId actl_fanout = min(fanout_i, edge_nums);
            // printf("col %d fanout %d", column_offset[id + 1] - column_offset[id], actl_fanout);
            assert(column_offset[id + 1] - column_offset[id] == actl_fanout);
            ...
            if (fanout_i < edge_nums) {
              ...
              while (sampled_idxs.size() < fanout_i) {
                // sampled_idxs.insert(rand_int(fanout_i));
                // sampled_idxs.insert(rand_int_seed(fanout_i));
                // int nbr_idx =  random_uniform_int(0, edge_nums - 1);
                // if()
                sampled_idxs.insert(random_uniform_int(0, edge_nums - 1));
              }
      ...
      sample_processing_time += get_time();
      // LOG_DEBUG("sample_one layer %d processing done", i);
      // printf("sample one sample_processing_time: %.4lf\n",sample_processing_time);
      // whole_graph->SyncAndLog("sample_processing");
      sample_post_time -= get_time();
      // ssg->sample_postprocessing();
      ssg->sample_postprocessing(sample_bits, i, node_idx);
    ...
    // sample_weight_time += weight_time;
    sample_compute_weight += get_time();
    // printf("sample_compute_weight: %.4lf sample_convert_graph_time: %.4lf sample_post_time: %.4lf sample_processing_time: %.4lf sample_init_co: %.4lf sample_load_dst: %.4lf\n",sample_compute_weight,sample_convert_graph_time,sample_post_time,sample_processing_time,sample_init_co,sample_load_dst);
  
  }
//   const int* is_sketch=nullptr, GNNDatum* gnnDatum=nullptr
  void sample_one_by_label(SampledSubgraph* ssg, int type = 0, bool phase = true,const std::vector<std::vector<std::pair<int,float>>> &nid_nbr = std::vector<std::vector<std::pair<int,float>>>() ) {
    assert(work_offset < work_range[1]);
    // assert(batch_size == batch_size_);
    // printf("sample_bs: %d",batch_size);
    VertexId actl_batch_size = std::min(batch_size, work_range[1] - work_offset);
    ...
    for (int i = 0; i < layers; i++) {
      ...
      ssg->sample_processing(
          [&](VertexId fanout_i, VertexId dst, VertexId* column_offset, VertexId* row_indices, VertexId id) {
            ...
            ///////////// no sorted_idxs copy //////////////////
            int pos = column_offset[id];
            // printf("sampled dst %d edge nums: %d\n",dst,edge_nums);
            if (fanout_i < edge_nums) {
              std::unordered_set<size_t> sampled_idxs;
              // std::vector<std::pair<int,float>>nid_sim;
              // for(int i=whole_offset[dst],j=0;i<whole_offset[dst+1];i++,j++) {
              //   nid_sim.push_back(std::make_pair(j,whole_graph->sim_value[i]));
              // }
              // sort(nid_sim.begin(), nid_sim.end(), [&](auto& x, auto& y) { return x.second > y.second; });
              int index = 0;
              // printf("sampled dst %d nbr num: %d edge nums: %d\n",dst,nid_sim.size(),edge_nums);
              while (sampled_idxs.size() < fanout_i) {
                // sampled_idxs.insert(nid_sim[index++].first);
                if(i==0) {
                  sampled_idxs.insert(nid_nbr[dst][index++].first);
                } else {
                  sampled_idxs.insert(random_uniform_int(0, edge_nums - 1));
                }
                

                // auto nbr_idx = random_uniform_int(0, edge_nums - 1);
                // if(gnnDatum->local_mask[whole_indices[whole_offset[dst] + nbr_idx]]==0) {
                //   if(is_sketch[whole_indices[whole_offset[dst] + nbr_idx]]==1) {
                //   sampled_idxs.insert(nbr_idx);
                //   // printf("%d ",nbr_idx);
                //   }
                // } else {
                //   sampled_idxs.insert(nbr_idx);
                //   // printf("%d ",nbr_idx);
                // }
                // printf("nbr class: %d \n",gnnDatum->local_label[whole_indices[whole_offset[dst] + nbr_idx]]);
                // if(gnnDatum->local_label[dst] == gnnDatum->local_label[whole_indices[whole_offset[dst] + nbr_idx]]) {
                //   sampled_idxs.insert(nbr_idx);
                // }
                ...
                // sampled_idxs.insert(rand_int_seed(fanout_i));
                // sampled_idxs.insert(random_uniform_int(0, edge_nums - 1));
                // // printf("test sample pr %d",whole_indices[whole_offset[dst] + nbr_idx]);
                // if(gnndatum->local_label[whole_indices[whole_offset[dst] + nbr_idx]]==gnndatum->local_label[dst]) {
                //   sampled_idxs.insert(nbr_idx);
                // }
      ...
      //         }
      //     });
      // printf("after proce: %.4lf\n",tmp_time+get_time());
      sample_processing_time += get_time();
      // LOG_DEBUG("sample_one layer %d processing done", i);
      // printf("sample_processing_time: %.4lf\n",sample_processing_time);
      // whole_graph->SyncAndLog("sample_processing");
      sample_post_time -= get_time();
      // ssg->sample_postprocessing();
      ssg->sample_postprocessing(sample_bits, i, node_idx);
  ...
  SampledSubgraph* ConstructSampledSubgraph_test(int layers, std::vector<VertexId>& layer_sizes,
                                            std::vector<VertexId>& node_mapping) {
    // ssg->sampled_sgs.clear();
    auto indptr = whole_graph->column_offset;
    auto indices = whole_graph->row_indices;
    SampledSubgraph* ssg = new SampledSubgraph();
    VertexId curr = 0;
    ...
    auto process_function = [this](VertexId element) {
        return whole_graph->local_global[element];
    };
    ...
    for (int i = 0; i < layers; ++i) {
      ...
      std::vector<VertexId> sources;
      // TODO(sanzo): redundancy copy
      std::transform(node_mapping.begin() + curr, node_mapping.begin() + curr + src_size, std::back_inserter(sources),process_function);
      // std::copy(node_mapping.begin() + curr, node_mapping.begin() + curr + src_size, std::back_inserter(sources));
      for (int j = 0; j < src_size; ++j) {
        source_map.insert(std::make_pair(node_mapping[curr + j], j));
      }
      ...
      std::vector<VertexId> destination;
      // TODO(sanzo): redundancy copy
      std::transform(node_mapping.begin() + curr + src_size, node_mapping.begin() + curr + src_size + dst_size,
                std::back_inserter(destination),process_function);
      // std::copy(node_mapping.begin() + curr + src_size, node_mapping.begin() + curr + src_size + dst_size,
      //           std::back_inserter(destination));      
      std::vector<VertexId> column_offset;
      column_offset.push_back(0);
...
void ClusterGCNSample(int layers, int batch_size, int partition_num, std::string objtype = "cut") {
        ...
        int pre = 0;
        int batch_num = 0;
        if(partition_num%batch_size!=0) {
            batch_num = partition_num/batch_size + 1;
        } else {
            batch_num = partition_num/batch_size;
        }
        printf("batch nums: %d\n",batch_num);
        int sum = 0;
        for(int j=0; j<partition_num/batch_size; j++) {
            node_set.clear();
            
            for (int i = 0; i < batch_size; ++i) {
                if(i + pre < partition_num) {
                    auto select_node = random_partition[i+pre];
                    sum += offset[select_node + 1] - offset[select_node ];

                    // for(int j = offset[select_node]; j<offset[select_node + 1]; j++) {
                    //   node_set.insert(whole_graph->local_global[ids[j]]);
                    // }
            ...
            // std::cout<<"partition sizes:"<<sum<<endl;
            // std::cout<<"node set sizes:"<<node_set.size()<<endl;
            vector<VertexId> node_ids(node_set.begin(), node_set.end());
            // std::cout << "select nodes node_ids " << node_ids.size() << std::endl;
            ...
            SampledSubgraph* ssg = ConstructSampledSubgraph_test(layers, actl_layer_sizes, node_mapping);
            // std::cout<<ssg->sampled_sgs[0]->v_size<<' '<<ssg->sampled_sgs[0]->src_size<<' '<<ssg->sampled_sgs[1]->v_size<<' '<<ssg->sampled_sgs[1]->src_size<<endl;
            push_one(ssg);            
            pre += batch_size;
            // printf("ssg edges: %d\n",ssg->sampled_sgs[0]->e_size);
        }
  ...
  void ClusterGCNSample_test(FullyRepGraph *ful_graph,int layers, int batch_size, int partition_num, std::string objtype = "cut") {
    if (metis_partition_id.empty()) {
      double metis_time = -get_time();
      // (FIXME Sanzo) store metis partion result to file
      MetisPartitionGraph(ful_graph, partition_num, objtype, metis_partition_id, metis_partition_offset);
      metis_time += get_time();
      printf("metis partition cost %.3f\n", metis_time);
    }
