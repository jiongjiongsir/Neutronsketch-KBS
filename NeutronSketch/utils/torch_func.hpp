...
float f1_score(NtsVar &input, NtsVar &target, bool onelabel = true) {
  ...
  if (!onelabel) {
    ...
    std::cout<<"pre: "<<predict[2]<<endl;
    predict = torch::where(predict > 0.5, 1, 0);
    // std::cout<<"input: "<<input[2]<<endl;
    
    // std::cout<<"pre: "<<predict.sum()<<endl;
    auto all_pre = predict.sum();
    // std::cout<<"all_pre: "<<all_pre<<endl;
    ...
    // std::cout<<"x_tmp: "<<x_tmp[2]<<endl;
    auto true_p = (x_tmp == predict).sum();
    std::cout<<"true_p: "<<true_p<<endl;
    auto all_true = target.sum();
    std::cout<<"all_true: "<<all_true<<endl;
    ...
    std::cout<<"pre: "<<precision<<"recall: "<<recall<<endl;
