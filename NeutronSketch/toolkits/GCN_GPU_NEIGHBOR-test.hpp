...
// class GCN_GPU_NEIGHBOR_TEST_impl {
//  public:
//   int iterations;
...
//   GCN_GPU_NEIGHBOR_TEST_impl(Graph<Empty>* graph_, int iterations_, bool process_local = false,
...
//     graph->config->cache_rate = 1;
//     graph->config->cache_policy == "degree";
...
//     // std::string suffix = "sample-rate-1.0.pt";
//     std::string suffix = "fanout-4,4.pt";
//     loadW(suffix);
...
class GCN_GPU_NEIGHBOR_TEST_impl {
 public:
  ...
  std::string dataset_name;
  torch::nn::Dropout drpmodel;
  long high_degree_num = 0;
  long low_degree_num = 0;
  long high_correct_num = 0;
  long low_correct_num = 0;
  // double sample_cost = 0;
  std::vector<torch::nn::BatchNorm1d> bn1d;
...
long get_correct1(NtsVar &input, NtsVar &target, std::vector<VertexId>& dst) {
  long ret = 0;
  // if (!onelabel) {
  ...
  // } else {
    ...
    int tmp_low = 0;
    for (auto v : dst) {
      // int edge_num = sampler->whole_graph->column_offset[v+1] - sampler->whole_graph->column_offset[v];
      // assert(graph->in_degree[v] == edge_num);
      int edge_num = graph->in_degree[v];
      if (edge_num > graph->config->target_degree) {
        high_degree_num++;
        if (output[idx].item<long>() == 1) {
          tmp_high++;
        }
      } else{
        low_degree_num++;
    ...
    LOG_DEBUG("all %d all_correct %d high_correct %d low_correct %d", dst.size(), ret, tmp_high, tmp_low);
    assert (tmp_high + tmp_low == ret);
    high_correct_num += tmp_high;
    low_correct_num += tmp_low;
  // }
  return ret;
}
  ...
  float EvalForward(Sampler* sampler, int type = 0) {
    ...
    while (sampler->work_offset < sampler->work_range[1]) {
      ...
      if (graph->config->classes == 1) {
        long ret_corr =  get_correct1(X[layers], target_lab, ssg->sampled_sgs.back()->dst());
        correct += ret_corr;
        train_nodes += target_lab.size(0);
    ...
    LOG_DEBUG("high node %d correct %d ratio %.3f", high_degree_num, high_correct_num, 1.0 * high_correct_num / high_degree_num);
    LOG_DEBUG("low node %d correct %d ratio %.3f", low_degree_num, low_correct_num, 1.0 * low_correct_num / low_degree_num);
    return acc;
  }
...
void count_sample_hop_nodes(Sampler* sampler) {
  ...
  float run() {
    ...
    for (int i_i = 0; i_i < iterations; i_i++) {
      ...
      suffix = "-1024fanout-4,4.pt";
      loadW(suffix);
      tmp_val_acc = EvalForward(eval_sampler, 1);
      LOG_DEBUG("after loadW %s best_val_acc %.3f\n", suffix.c_str(), tmp_val_acc);
      

      
      
      suffix = "-1024fanout-8,8.pt";
      loadW(suffix);
      tmp_val_acc = EvalForward(eval_sampler, 1);
      LOG_DEBUG("after loadW %sbest_val_acc %.3f\n", suffix.c_str(), tmp_val_acc);
      

      suffix = "-1024fanout-16,16.pt";
      loadW(suffix);
      tmp_val_acc = EvalForward(eval_sampler, 1);
      LOG_DEBUG("after loadW %s val_acc %.3f best_val_acc %.3f\n", suffix.c_str(), tmp_val_acc, best_val_acc);
