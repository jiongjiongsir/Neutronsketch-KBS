...
class GCN_GPU_NEIGHBOR_Select_impl {
 public:
  int iterations;
  ...
  GCN_GPU_NEIGHBOR_Select_impl(Graph<Empty>* graph_, int iterations_, bool process_local = false,
...
#pragma omp parallel for
  ...
  void zerocopy_version(Sampler* sampler) {
    ...
    while (sampler->work_offset < sampler->work_range[1]) {
      ...
      auto ssg = sampler->subgraph;
      epoch_sample_time -= get_time();
      // LOG_INFO("Before metis_batch_id:%d",sampler->metis_batch_id);
      sampler->sample_one(ssg, graph->config->batch_type, ctx->is_train());
      ...
      //   tmp = tmp+result.size();
      // } else {
      //   printf("6000 batchs unique nodes nums: %d\n",tmp);
      //   break;
      // }

      ofstream  file;
      std::string suffix;
      switch (graph->config->batch_type)
      {
      case 0:
        suffix = "shuffle";
        break;
      ...
      file.open("/home/yuanh/neutron-sanzo/log/batch-type/reddit-batch/"+suffix+"/tmp"+to_string(i)+".log");
      ...
      // std::vector<int>tmp_nbr;
      // int tmp_nid = 0;
      // for(int i=0;i<d_0.size();i++)
      // {
      //   tmp_nid = d_0[i];
      //   tmp_nbr.clear();
      //   for(int j=fully_rep_graph->column_offset[tmp_nid];j<fully_rep_graph->column_offset[j+1];j++)
      //   {
      //     int nbr_nid = fully_rep_graph->row_indices[j];
      //     tmp_nbr.push_back(nbr_nid);
      //   }
      //   for(int j=0;j<s_0.size();j++)
      //   {
      //     if(find(tmp_nbr.begin(), tmp_nbr.end(), s_0[j])!=tmp_nbr.end()){
      //       edge_list.push_back(make_pair(tmp_nid,s_0[j]));
      //     }
      //   }
      // }
      ...
      //   {
      //     if(find(tmp_nbr.begin(), tmp_nbr.end(), s_1[j])!=tmp_nbr.end()){
      //       edge_list.push_back(make_pair(tmp_nid,s_1[j]));
      //     }
      //   }
      // }
      // printf("edge nums: %d",edge_list.size());

      // for(int i=0;i<layers;i++)
      // {
      //   auto p = ssg->sampled_sgs[i]->src();
      //   auto d = ssg->sampled_sgs[i]->dst();
      //   printf("layer:%d src:%d dst:%d\n",i,p.size(),d.size());
      // }
    ...
    // VertexId node_size = sampler->subgraph->sampled_sgs[0]->v_size + sampler->subgraph->sampled_sgs[1]->v_size;
    
    // printf("Epoch nodes nums : %d %d\n",epoch_nodes.size(),node_size);
    // std::set<VertexId>s(epoch_nodes.begin(),epoch_nodes.end());
    // epoch_nodes.assign(s.begin(),s.end());
    // printf("Epoch nodes nums unique: %d %d\n",epoch_nodes.size(),node_size);
    // // VertexId edge_size = sampler->subgraph->sampled_sgs[0]->e_size + sampler->subgraph->sampled_sgs[1]->e_size;
    // printf("Epoch edge size: %d\n",edge_size);
    ...
    //     break;
    // }     
    // file.open("/home/yuanh/neutron-sanzo/log/batch-type/ogbn-arxiv-batch/"+suffix+"/epoch_nodes.log",ios::out);
    //   for(const auto&v:epoch_nodes){
    //     file << v <<" ";
    //   }
    //   file.close();
  ...
  std::pair<float, double> Forward(Sampler* sampler, int type = 0) {
    ...
    // if(graph->config->batch_type == METIS) {
    //   // shuffle_vec_seed(sampler->partition_nodes);
    //   // for(int i=0;i<sampler->partition_nodes.size();i++)
    //   // {
    //   //   shuffle_vec_seed(sampler->partition_nodes[i]);
    //   // }
    //   double get_batch_time = -get_time();
    //   sampler->get_metis_batch();
    //   get_batch_time += get_time();
    //   printf("shuffle and get batch time: %f\n",get_batch_time);
    // }

    // for(int i = 0;i<100;i++)
    // {
    //   LOG_INFO("nids:%d degree:%d",sampler->sample_nids[i],graph->out_degree[sampler->sample_nids[i]]);
    // }
    // node sampling
    ...
    // old_version(sampler);
    // LOG_INFO("graph->config->mode %s",graph->config->mode);
...
void count_sample_hop_nodes(Sampler* sampler) {
  ...
  float run() {
    ...
    if (batch_type == METIS) {
      ...
      // int partition_num = graph->config->partition_num;
    ...
    for (int i_i = 0; i_i < iterations; i_i++) {
      ...
      //   stop_flag++;
      //   if(stop_flag == 50){
      //     LOG_INFO("Best Epoch:%d Best Val ACC:%.3f",best_epoch,best_val_acc);
      //     break;
      //   }
      // }
