...
#pragma omp parallel for num_threads(threads)
  ...
  float Forward(Sampler* sampler, int type = 0) {
    ...
    get_gpu_mem(used_gpu_mem, total_gpu_mem);
    LOG_INFO(
        "Epoch %03d epoch_time %.3f sample_time %.3f gather_time %.3f trans_time %.3f train_time %.3f (%.3f-%.3f) "
        ...
        epoch_train_time, epoch_train_graph_time, epoch_train_graph_time, 
        graph->config->cache_rate, epoch_cache_hit_rate, epoch_trans_memory, used_gpu_mem);
    post_time += get_time();
