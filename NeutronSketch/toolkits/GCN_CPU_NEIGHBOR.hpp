...
#pragma omp parallel for
  ...
  float run() {
    ...
    for (int i_i = 0; i_i < iterations; i_i++) {
      tmp = P[layers-1]->W.clone();
      graph->rtminfo->epoch = i_i;
      // cout<<"before up:"<<P[layers-1]->W<<endl;
      ctx->train();
      ...
      // print("train TIME%f",P[layers]->W);
      // c10::IntArrayRef t_size = P[layers]->W.sizes();
      // std::cout<<(P[layers-1]->W).sizes()<<endl;
      // cout<<"after up:"<<P[layers-1]->W<<endl;
      
      res = (P[layers-1]->W - tmp).flatten();
      
      // std::cout<<res<<endl;
      float *c = res.data_ptr<float>();
      float sum = 0;
      for(int m = 0;m<128*7;m++)
      {
        if(c[m]<0)
        {
          sum = sum - c[m];
